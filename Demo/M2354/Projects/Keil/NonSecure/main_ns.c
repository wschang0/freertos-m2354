...
extern int32_t ns_interface_lock_init();

void regression_test()
{
    enum test_suite_err_t err;
    
    err = tfm_non_secure_client_run_tests();
...
int main( void )
{
    ...
    xTaskCreate(regression_test, "test", 8192, (void *) NULL, tskIDLE_PRIORITY + 1, NULL);
